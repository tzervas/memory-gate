...
class MetricsRecorder:
    ...
    def __init__(self, metrics_file: str = "test_metrics.json", display_config: Optional[Dict] = None):
    ...
    def calculate_statistics(self, metric_category: str, metric_name: str) -> Dict[str, float]:
    ...
    def get_trend(self, metric_category: str, metric_name: str, last_n: int = 5) -> str:
        ...
        recent_runs = self.previous_runs[-last_n:] if len(self.previous_runs) >= last_n else self.previous_runs
        values = []
    ...
    def generate_report(self) -> str:
        ...
        if self.metrics["errors"]:
            ...
            for error in self.metrics["errors"]:
                report_sections.append(f"  • {error['test']}: {error['type']} - {error['message']}")
            ...
            for name, value in self.metrics["timing"].items():
                ...
                if stats["count"] > 0:
                    if stats["mean"] != 0:  # Prevent division by zero
                        ...
                        current_vs_avg = 0
                    trend_symbol = "📈" if current_vs_avg > 5 else "📉" if current_vs_avg < -5 else "➡️"
                    report_sections.append(f"  {trend_symbol} {name}: {self.format_duration(value)} (avg: {self.format_duration(stats['mean'])}, {current_vs_avg:+.1f}%)")
        ...
        if self.display_config["show_trends"] and len(self.previous_runs) >= 2:
            ...
            for name in self.metrics["timing"].keys():
                ...
                trend_emoji = {"increasing": "📈", "decreasing": "📉", "stable": "➡️", "insufficient_data": "❓"}[trend]
    ...
    def get_summary_metrics(self) -> Dict[str, Any]:
        ...
        if "Total Run Time" in self.metrics["timing"]:
            summary["total_time"] = self.format_duration(self.metrics["timing"]["Total Run Time"])
