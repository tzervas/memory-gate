...
class VectorMemoryStore(KnowledgeStore[LearningContext]):
    ...
    def __init__(
        ...
        if persist_directory:
            self.client = chromadb.PersistentClient(
                ...
                settings=Settings(**(chroma_settings or {"anonymized_telemetry": False}))
            )
        else:
    ...
    async def retrieve_context(
        ...
        """
        try:
            with MEMORY_RETRIEVAL_LATENCY_SECONDS.labels(store_type="vector_store").time():
            ...
            if query_results['ids'] and query_results['ids'][0]: # Check if there are any results
                for i, doc_content in enumerate(query_results['documents'][0]):
                    ...
                    contexts.append(LearningContext(
                        content=doc_content,
                        domain=str(metadata.get('domain', 'unknown')), # handle missing domain
                        timestamp=datetime.fromisoformat(str(metadata['timestamp'])),
                        importance=float(str(metadata.get('importance', 1.0))), # handle missing importance
    ...
    async def get_experience_by_id(self, key: str) -> Optional[LearningContext]:
        ...
        try:
            with MEMORY_RETRIEVAL_LATENCY_SECONDS.labels(store_type="vector_store").time():
                result = self.collection.get(ids=[key], include=["metadatas", "documents"])

            if not result['ids'] or not result['documents']:
                record_memory_operation(operation_type="get_experience_by_id", success=True) # Success, but not found
                return None
            ...
            record_memory_operation(operation_type="get_experience_by_id", success=False)
    ...
    async def get_experiences_by_metadata_filter(
        ...
        """
        try:
            with MEMORY_RETRIEVAL_LATENCY_SECONDS.labels(store_type="vector_store").time(): # Classify as retrieval
            ...
            if results['ids'] and results['documents'] and results['metadatas']: # Ensure all lists are present and non-empty
            ...
            record_memory_operation(operation_type="get_experiences_by_metadata_filter", success=True)
            return items
        except Exception as e:
            record_memory_operation(operation_type="get_experiences_by_metadata_filter", success=False)
...
async def main_test() -> None:
    ...
    vector_store = VectorMemoryStore(collection_name="test_run_collection", persist_directory=None)
    ...
    for ctx in retrieved_contexts:
        print(f"  - Content: {ctx.content}, Domain: {ctx.domain}, Importance: {ctx.importance}, Meta: {ctx.metadata}")

    print("\nRetrieving context for 'deployment script' with domain 'devops':")
    retrieved_devops = await vector_store.retrieve_context("deployment script", limit=1, domain_filter="devops")
    for ctx in retrieved_devops:
        print(f"  - Content: {ctx.content}, Domain: {ctx.domain}, Importance: {ctx.importance}")
